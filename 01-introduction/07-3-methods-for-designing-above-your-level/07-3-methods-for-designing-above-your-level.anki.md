...
```mermaid
flowchart TD
    A[Designing Above<br/>Your Level] --> B[1. Fail Left,<br/>Fail Right]
    A --> C[2. Julie's Method]
    A --> D[3. Sleep On It]
    B --> B1[Explore extremes to<br/>find the acceptable range]
    C --> C1[Try every variation<br/>you can think of]
    D --> D1[Come back tomorrow<br/>with fresh eyes]
```
![](images/07-3-methods-for-designing-above-your-level-card-1.png)
...
```mermaid
graph LR
    A["Too Small<br/>(e.g. 24px)"] --- B["Acceptable Range<br/>(e.g. 30–50px)"] --- C["Too Big<br/>(e.g. 100px)"]
    style A fill:#c0392b,color:#ffffff
    style B fill:#1a7a3c,color:#ffffff
...
```mermaid
flowchart LR
    subgraph Hard["Requires Sustained Mental Effort"]
        B[Fail Left,<br/>Fail Right]
        C[Julie's Method]
    end
    subgraph Easy["Boring but True"]
        D[Sleep On It]
    end
```
![](images/07-3-methods-for-designing-above-your-level-card-3.png)
