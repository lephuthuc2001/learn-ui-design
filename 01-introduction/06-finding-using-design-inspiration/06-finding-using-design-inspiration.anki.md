...
flowchart LR
    ...
    A --> C[Behance]
    A --> D["Screenshot sites<br/>(Mobbin, Pttrns)"]
    A --> E[Live websites]
```
![](images/06-finding-using-design-inspiration-card-1.png)
...
```mermaid
flowchart TD
    A[Found a great design?] --> B{Source}
    B -->|Dribbble| C[Save to Collection<br/>online only]
    B -->|Screenshot / image| D[Download to local<br/>Good UI folder]
    B -->|Live site| E[Bookmark in<br/>Good UI folder]
    D --> F[Accessible offline]
    E --> G[Full interaction<br/>+ WhatFont available]
```
![](images/06-finding-using-design-inspiration-card-2.png)
...
```mermaid
flowchart LR
    A["Day 1:<br/>Find + analyze<br/>inspiration"] --> B[Sleep]
    B --> C["Day 2+:<br/>Design without<br/>looking at original"]
    C --> D["Output: lessons you<br/>remember, expressed<br/>in your own way"]
```
![](images/06-finding-using-design-inspiration-card-3.png)
