...
When the user asks to publish a finished teardown, read [references/publishing-teardown.md](references/publishing-teardown.md) for the full flow: the `.mdx` scaffold with teardown components, image-path rewriting, build verification, and the two-repo commit. Don't load it while writing the notes `.md` — only when publishing is requested.
